...
import showdown.model.CreditCardType;
import showdown.model.Dvd;
import showdown.model.Purchase;
...
public class Checkout extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		Calendar expirationDate = new GregorianCalendar(cardYear,
				cardMonth - 1, 1);
		...
		if (cardNumber == null || cardNumber.length() == 0) {
			...
			dispatcher = setupErrorPage(request,
					"Card number can only contain digits with no spaces");
			dispatcher.forward(request, response);
			...
			String postfix = cardNumber.substring(cardNumber.length() - 4,
					cardNumber.length());
			Collection<Dvd> dvds = (Collection<Dvd>) request.getSession()
					.getAttribute("dvds");
			Purchase purchase = new Purchase(type, postfix, expirationDate,
					billingName, address, city, state, zip, dvds);
			request.getSession().setAttribute("purchase", purchase);

			// delete shopping cart
			User user = ShowdownDB.readUser(request.getUserPrincipal()
					.getName());
			ShowdownDB.deleteShoppingCart(user);

			// set attributes
			DateFormat dateFormat = new SimpleDateFormat("MM/yy");
			request.getSession().setAttribute("expirationDate",
					dateFormat.format(expirationDate.getTime()));
